...
<div class="modal-backdrop" id="{{ $modalId }}"
     ...
     @if($checkMonthUrl) data-check-month-url="{{ $checkMonthUrl }}" @endif
     @if($checkYearUrl)  data-check-year-url="{{ $checkYearUrl }}"   @endif
     @if($hideFormOnFile) data-hide-form-on-file="1"                 @endif>
                ...
                <div class="import-preview-form" data-role="form-section">
                    ...
                    @if(!empty($lastUpload))
                        <div class="import-last-upload-card" data-role="last-upload-card">
                            <div class="last-upload-container">
                                <div class="last-upload-info">
                                    ...
                                    <div class="import-last-upload-meta">
                                        <div class="meta-row"><strong>File:</strong> {{ $lastUpload['file_name'] ?? '' }}</div>
                                        <div class="meta-date">Uploaded: {{ $lastUpload['uploaded_at'] ?? '' }}</div>
                                    </div>
                                </div>
                            ...
                            <div data-role="last-upload-status"
                                 style="display:none;font-size:.78rem;color:var(--mid);margin-top:8px;padding-top:8px;border-top:1px solid var(--border);"></div>
                        </div>
                    @endif

                    {{-- Drop zone --}}
                    <div class="import-drop-zone" data-role="drop-zone">
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--sage)"
                             stroke-width="1.5" style="margin:0 auto 10px;display:block;">
                            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                            <polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
                        </svg>
                        <div data-role="drop-label" data-default="{{ $dropLabel }}"
                             style="font-size:.9rem;font-weight:500;color:var(--ink);margin-bottom:4px;">
                            {{ $dropLabel }}
                        </div>
                        <div style="font-size:.8rem;color:var(--mid);">Auto-preview starts after upload</div>
                        <input type="file" data-role="file-input" name="spreadsheet"
                               accept="{{ $accept }}" style="display:none">
                    </div>

                    <div class="import-file-info" data-role="file-info">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--leaf)"
